...
from calibration.calibration import apply_calibration
from provenance.model_cards import model_cards_index
...
class Evaluator:
    def __init__(self, *, cache_enabled: bool = True, cache_max: int = 256):
        # Read-only acceleration. Must not change numerical results.
    ...
    Today, this wraps `hot_ion_point`. Over time, this is the natural place to:
      - add caching
      - add hybrid analytic/numeric Jacobians
      - add systematic variable/output scaling
      - emit standardized diagnostics artifacts
    """

    def __init__(self, *, label: str = "hot_ion_point"):
        self.label = label
        # Small memoization cache for repeated evaluations in scans/optimizations
