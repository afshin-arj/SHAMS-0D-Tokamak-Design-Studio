...
def classify_severity(old_artifact: Dict[str, Any], new_artifact: Dict[str, Any], diff: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for c in removed:
        if not isinstance(c, dict):
            ...
            name = c.get("name")
            meta = {k: c.get(k) for k in _CONSTRAINT_META_FIELDS}
        if name in renamed_old:
            continue
        findings.append({"severity":"breaking","kind":"constraint_removed","detail":{"name":name, "meta":meta}})
    ...
    for c in added:
        ...
        if sev in ("hard","required"):
            severity = "breaking"
        findings.append({"severity":severity,"kind":"constraint_added","detail":{"name":name, "meta":meta}})
