...
def _lts_Bc2_T(Bc20_T: float, T_K: float, Tc_K: float, p: float = 1.7) -> float:
    """Very lightweight upper-critical-field proxy for Nb-based LTS.

    Bc2(T) ≈ Bc20 * (1 - (T/Tc)^p)
    """
    if T_K >= Tc_K:
        return 0.0
    x = max(min(T_K / max(Tc_K, 1e-9), 1.0), 0.0)
    return max(Bc20_T * (1.0 - x**p), 0.0)


def lts_Jc_norm(B_T: float, T_K: float, *, Tc_K: float, Bc20_T: float, B0_T: float, n_T: float = 1.5, m_B: float = 2.0) -> float:
    """Normalized NbTi/Nb3Sn-like critical engineering current density proxy.

    - Jc→0 when T→Tc or B→Bc2(T)
    - monotonic in B and T
    """
    if T_K >= Tc_K:
        return 0.0
    Bc2 = _lts_Bc2_T(Bc20_T, T_K, Tc_K)
    if B_T >= Bc2:
        return 0.0
    temp = max(1.0 - T_K / Tc_K, 0.0) ** n_T
    # penalize as B approaches Bc2
    frac = max(min(B_T / max(Bc2, 1e-9), 0.999999), 0.0)
    near = max(1.0 - frac, 1e-12) ** 1.0
    field = 1.0 + (max(B_T, 0.0) / max(B0_T, 1e-9)) ** m_B
    return temp * near / field


def tf_sc_flag(magnet_technology: str) -> float:
    tech = str(magnet_technology or '').strip().upper()
    return 0.0 if tech == 'COPPER' else 1.0


def sc_Jc_norm(B_T: float, T_K: float, magnet_technology: str) -> float:
    tech = str(magnet_technology or '').strip().upper()
    if tech in ('HTS_REBCO', 'HTS', 'REBCO'):
        return rebco_Jc_norm(B_T, T_K)
    if tech in ('LTS_NB3SN', 'NB3SN'):
        # Nb3Sn anchors: Tc~18.3K, Bc2(0)~28T (proxy)
        return lts_Jc_norm(B_T, T_K, Tc_K=18.3, Bc20_T=28.0, B0_T=12.0)
    if tech in ('LTS_NBTI', 'NBTI'):
        # NbTi anchors: Tc~9.2K, Bc2(0)~14.5T (proxy)
        return lts_Jc_norm(B_T, T_K, Tc_K=9.2, Bc20_T=14.5, B0_T=6.0)
    if tech == 'COPPER':
        return float('nan')
    # Default: preserve legacy behavior
    return rebco_Jc_norm(B_T, T_K)


def sc_operating_margin(Bpeak_T: float, Tcoil_K: float, magnet_technology: str,
                        strain: float = 0.0, strain_crit: float = 0.004,
                        B_ref_T: float = 20.0, Jc_mult: float = 1.0) -> float:
    """Unified superconducting margin proxy keyed by magnet technology.

    For COPPER returns NaN (not applicable).
    """
    tech = str(magnet_technology or '').strip().upper()
    if tech == 'COPPER':
        return float('nan')
    Jc = sc_Jc_norm(Bpeak_T, Tcoil_K, tech)
    try:
        f_strain = math.exp(- (float(strain) / max(float(strain_crit), 1e-9)) ** 2) if float(strain) != 0.0 else 1.0
    except Exception:
        f_strain = 1.0
    Jop = max(Bpeak_T / max(B_ref_T, 1e-9), 1e-12)
    return (float(Jc_mult) * Jc * f_strain) / Jop


def copper_tf_ohmic_power_MW(Jop_A_m2: float, wp_area_m2: float, R0_m: float,
                             rho_ohm_m: float = 1.7e-8, length_factor: float = 1.0) -> float:
    """Very lightweight resistive TF loss proxy.

    Uses power density p = rho * J^2, total P = p * Volume.
    Volume ~ area_wp * (2πR0) * length_factor.

    This is a screening proxy only; used to make copper technology visible in outputs.
    """
    V = max(float(wp_area_m2), 0.0) * (2.0 * math.pi * max(float(R0_m), 1e-6)) * max(float(length_factor), 0.1)
    P_W = float(rho_ohm_m) * (max(float(Jop_A_m2), 0.0) ** 2) * V
    return P_W / 1e6
...
def hts_operating_margin_tech(Bpeak_T: float, Tcoil_K: float, magnet_technology: str,
                              strain: float = 0.0, strain_crit: float = 0.004,
                              B_ref_T: float = 20.0, Jc_mult: float = 1.0) -> float:
    return sc_operating_margin(Bpeak_T, Tcoil_K, magnet_technology, strain=strain, strain_crit=strain_crit, B_ref_T=B_ref_T, Jc_mult=Jc_mult)
