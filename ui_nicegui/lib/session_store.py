...
def _stamp_presentation_caches(session: DesignSession, outputs: Dict[str, Any]) -> None:
    """Precompute deck-switch display artifacts once per evaluation (UI only)."""
    summary = verdict_summary(outputs)
    summary["_cache_run_ts"] = session.pd_last_run_ts
    summary["_cache_intent"] = str(session.design_intent)
    session.pd_verdict_summary_cache = summary
    if summary.get("feasible"):
        session.pd_no_solution_atlas_cache = None
        return
    try:
        from ui_nicegui.lib.pd_parity_helpers import no_solution_atlas_summary

        atlas = no_solution_atlas_summary(
            outputs, design_intent=str(session.design_intent)
        )
        atlas["_cache_run_ts"] = session.pd_last_run_ts
        atlas["_cache_intent"] = str(session.design_intent)
        session.pd_no_solution_atlas_cache = atlas
    except Exception:
        session.pd_no_solution_atlas_cache = None


def _cache_matches_session(session: DesignSession, cache: Optional[Dict[str, Any]]) -> bool:
    return (
        isinstance(cache, dict)
        and cache.get("_cache_run_ts") == session.pd_last_run_ts
        and cache.get("_cache_intent") == str(session.design_intent)
        and session.pd_last_run_ts is not None
    )


def get_cached_verdict_summary(session: DesignSession, out: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Return cached verdict when it matches the active point evaluation."""
    point = out if isinstance(out, dict) else (session.pd_last_outputs or session.last_eval)
    if not isinstance(point, dict) or not point:
        return {"loaded": False}
    cache = session.pd_verdict_summary_cache
    if _cache_matches_session(session, cache) and cache.get("loaded"):
        return cache
    summary = verdict_summary(point)
    summary["_cache_run_ts"] = session.pd_last_run_ts
    summary["_cache_intent"] = str(session.design_intent)
    session.pd_verdict_summary_cache = summary
    return summary
...
def get_cached_no_solution_atlas(session: DesignSession, out: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Return cached NO-SOLUTION atlas for the active evaluation (empty dict if feasible/missing)."""
    point = out if isinstance(out, dict) else (session.pd_last_outputs or session.last_eval)
    if not isinstance(point, dict) or not point:
        return {}
    cache = session.pd_no_solution_atlas_cache
    if _cache_matches_session(session, cache):
        return cache
    from ui_nicegui.lib.pd_parity_helpers import no_solution_atlas_summary

    atlas = no_solution_atlas_summary(point, design_intent=str(session.design_intent))
    atlas["_cache_run_ts"] = session.pd_last_run_ts
    atlas["_cache_intent"] = str(session.design_intent)
    session.pd_no_solution_atlas_cache = atlas
    return atlas
...
def set_point_evaluation(
    ...
    *,
    ...
    # Stamp plant KPI honesty once so Scan/Pareto/Trade captions skip rebuilds on deck switch.
    try:
        from ui_nicegui.lib.plant_kpi_honesty_ui import plant_kpi_honesty_for_point

        artifact["plant_kpi_honesty"] = plant_kpi_honesty_for_point(
            outputs, design_intent=str(session.design_intent)
        )
    except Exception:
        pass
...
def clear_point_designer(session: DesignSession) -> None:
    ...
    session.pd_verdict_summary_cache = None
    session.pd_no_solution_atlas_cache = None
