...
| 4 Independence | **In progress** | PROCESS = legacy only; next scoped retirement evidence report (4.1) — **no blanket retirement claim** |
...
15. **Next (Phase 4.1):** Scoped PROCESS retirement evidence report — `process_retirement_report` schema + generator scoped to domains with parity dossiers/champion evidence only (never blanket "PROCESS retired"); cite SHAMS VERSION + artifact hashes (`/process-specialist`, `/documentation`, `/reviewer`)
