...
def solve_for_targets(
    ...
    *,
    ...
    try:
        ...
        return SolveResult(base, base_eval.out, False, 0, "targets and variables must have same dimension", trace=[], report=_mk_report(backend="bounded_newton_fd", status="failed", message="targets and variables must have same dimension", iters=0, trace=[], out=base_eval.out, targets=targets, var_keys=var_keys, x=[variables[k][0] for k in var_keys], bounds=[(variables[k][1], variables[k][2]) for k in var_keys]))

    # --- Scaling (PROCESS-inspired) ---
    x = [float(variables[k][0]) for k in var_keys]
    trace = []  # iteration trace for auditability
...
def solve_for_targets_multistart(
    ...
    *,
    ...
    for cand in candidates[: max(2, restarts)]:
        ...
        res = solve_for_targets(base, targets, vars2, max_iter=max_iter, tol=tol, damping=damping)
