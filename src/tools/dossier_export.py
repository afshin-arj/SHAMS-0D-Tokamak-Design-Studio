...
def export_design_dossier_zip(
    ...
    *,
    ...
    with zipfile.ZipFile(out_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        # v309.0: materials admissibility snapshot (small, reviewer-friendly)
        try:
            nm_keys = [
                "TBR",
                "TBR_min",
                ...
                "neutron_attenuation_fast",
                "neutron_attenuation_gamma",
                "P_nuc_total_MW",
                "P_nuc_TF_MW",
                "P_nuc_PF_MW",
                "P_nuc_cryo_kW",
                ...
                "fw_sigma_allow_MPa",
                "fw_sigma_margin_MPa",
                "blanket_dpa_per_year",
                "blanket_He_appm_per_year",
                "blanket_lifetime_yr",
                "blanket_T_oper_C",
                "blanket_T_min_C",
                "blanket_T_max_C",
                "blanket_T_margin_C",
                "blanket_sigma_oper_MPa",
                "blanket_sigma_allow_MPa",
                "blanket_sigma_margin_MPa",
            ]
            outs = artifact.get("outputs") if isinstance(artifact.get("outputs"), dict) else artifact
            nm = {k: outs.get(k) for k in nm_keys if isinstance(outs, dict) and k in outs}
            if nm:
                add_json(zf, f"{basename}/materials_admissibility.json", nm)
        except Exception:
            pass
